...
#define LABEL_NALE_TXT_PATH "/root/rk3588/up_weight_label.txt"
using namespace std;
...
static int loadLabelName(const char *locationFilename, char *label[]) {
    ...
    while (fgets(buffer, sizeof(buffer), file) && index < OBJ_CLASS_NUM) {
        // 去除换行符
    ...
    // 验证加载的标签数量
    printf("Total loaded labels: %d\n", index);
    if (index == 0) {
        printf("Error: No valid labels loaded!\n");
        return -1;
    }
...
const char* coco_cls_to_name(int cls_id) {
    ...
    switch (cls_id) {
        case 0: return "1"; printf("label:1\n");
        case 1: return "2"; printf("label:2\n");
        case 2: return "3"; printf("label:3\n");
        case 3: return "4"; printf("label:4\n");
        case 4: return "5"; printf("label:5\n");
        case 5: return "6"; printf("label:6\n");
        case 6: return "sharp"; printf("label:sharp\n");
        case 7: return "tag";
...
// const char* coco_cls_to_name(int cls_id) {
//     // 调试输出
//     // printf("Querying class ID: %d\n", cls_id);
    
//     if (cls_id < 0 || cls_id >= OBJ_CLASS_NUM) {
//         printf("Invalid class ID: %d (max: %d)\n", cls_id, OBJ_CLASS_NUM-1);
//         return "invalid_id";
//     }
//     /*
//     if (!labels[cls_id]) {
//         printf("Label %d not loaded!\n", cls_id);
//         return "unloaded";
//     }
//     */
//     switch (cls_id){
